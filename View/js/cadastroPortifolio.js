...
document.addEventListener('DOMContentLoaded', function () {
    // Escopo petite-vue simples
    window.formCadastroPortifolio = function () {
        return {
            formData: {
                treino_idTreino: '',
                descricao: ''
            ...
            async cadastrarPortifolio() {
                try {
                    ...
                    // sucesso
                    const newId = json && json.id ? json.id : null;
    ...
    // Inicializar carregamento automático de treinos
    // Aguardamos petite-vue inicializar o escopo, então chamamos com pequeno timeout
    setTimeout(() => {
    ...
    // Garantia extra: se o form for submetido sem que petite-vue esteja ligado,
    // interceptamos o submit nativamente e chamamos a função de cadastro.
    // Isso resolve casos em que o binding do v-scope não tenha ocorrido.
    setTimeout(() => {
        ...
        if (form) {
            form.addEventListener('submit', function (e) {
                ...
                if (window.formCadastroPortifolio && typeof window.formCadastroPortifolio.cadastrarPortifolio === 'function') {
                    // sincroniza o valor do select com o objeto formData caso o v-model não esteja ativo
